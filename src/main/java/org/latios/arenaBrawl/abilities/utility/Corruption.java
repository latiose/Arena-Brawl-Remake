...
public class Corruption implements Ability {
    ...
    public Corruption(CooldownManager cooldownManager, TeamManager teamManager,
                      CombatUpgradeManager combatUpgradeManager, PlayerHealthManager playerHealthManager) {
    ...
    public boolean activate(Player player) {
        ...
        playerHealthManager.disableRegen(target, DURATION_MILLIS);
    ...
    private void startParticleEffect(Player target) {
        new BukkitRunnable() {
            @Override
            public void run() {
                ...
                if (!playerHealthManager.isRegenDisabled(target)) {
                    target.sendMessage(MessageUtils.positive() + "§3Corruption has worn off!");
                    cancel();
                    return;
                }
